...
### 非研发交互增强（v3）
1. **新手引导卡**：首次使用显示 4 个口语化示例（"每天把后台订单导到表格里"等），点击一键填入输入框；只显示一次（chrome.storage 记住）。
2. **口语化输入占位符**：默认提示"说说你想自动化的重复工作，比如：每天把订单导出来…"。
3. **探索明细**：探索横幅下实时列出"已学会的步骤"（✓/✗ + note），普通用户能看到 LLM 学到了什么，探索透明。
4. **实机验证**：360Chrome 加载扩展，引导卡显示/示例填入/占位符/探索明细容器全部通过（`test/onboarding-check.mjs`）。
